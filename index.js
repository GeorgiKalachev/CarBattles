...
function getUserData(){
    FB.api('/me', {fields: 'name,email,picture.type(large)'}, function(response) {
        ...
        fname.innerHTML = 'Welcome, ' + response.name; // Display the "Welcome" message along with the user's name
        ...
        var facebookStartGameButton = document.getElementById('facebookStartGameButton');
        facebookStartGameButton.addEventListener('click', function() {
            startGame();

            // Send user data to the server
            sendUserData(response.name, response.email, response.picture.data.url);
        });
    });
}

function sendUserData(name, email, pictureUrl) {
    // Make an AJAX POST request to the server
    fetch('http://localhost:5000/users/facebook-login', {
        method: 'POST',
        mode: 'no-cors',
        headers: {
...
function onSignIn(googleUser) {
    ...
    google.style.display = 'none';
    account.style.display = 'none';
    var googleStartGameButton = document.getElementById('googleStartGameButton');
    googleStartGameButton.addEventListener('click', startGame);
}
